...
export interface NtfyConfig {
  /** Full publish URL, e.g. https://ntfy.sh/<topic>. */
  url: string;
}
 ...
 */
export function parseNotifyConfig(raw: string | undefined): NtfyConfig {
  if (!raw || raw.trim() === "") {
    throw new Error("--notify is required when --auth is enabled (e.g. --notify ntfy:<topic>)");
  }
  let value = raw.trim();
  if (value.startsWith("ntfy:")) value = value.slice("ntfy:".length);
  if (value === "") {
...
export function createNtfyNotifier(config: NtfyConfig): Notifier {
  return async (payload: NotificationPayload): Promise<void> => {
