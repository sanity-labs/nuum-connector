...
export type LeaseDurationMs = number | null;
...
export function isValidFiniteLeaseMs(value: unknown): value is number {
  return typeof value === "number" && Number.isSafeInteger(value) && value > 0;
}

/** Validate a raw wire value: `null` (indefinite) or a valid finite number. */
export function isValidLeaseDurationMs(value: unknown): value is LeaseDurationMs {
 ...
 */
export function leaseExpiryAt(atMs: number, durationMs: number): number | null {
  const expiresAt = atMs + durationMs;
  if (!Number.isSafeInteger(expiresAt) || Number.isNaN(new Date(expiresAt).getTime())) return null;
  return expiresAt;
}

export interface ParseLeaseDurationOptions {
  /** Accept the literal word `indefinitely` (→ null). Default false. */
  allowIndefinite?: boolean;
  /** Flag name used in error messages, e.g. "--lease". */
  flag: string;
}
 ...
 * `validateLeasePolicy`).
 */
export function parseLeaseDuration(raw: string, opts: ParseLeaseDurationOptions): LeaseDurationMs {
  const value = raw.trim();
  const allowed = opts.allowIndefinite
    ? "use e.g. 30m, 2h, 8h, 2d, or 'indefinitely'"
    : "use e.g. 30m, 2h, 8h, 2d";
  if (value === INDEFINITELY) {
    if (opts.allowIndefinite) return null;
 ...
 */
export function formatLeaseDuration(ms: LeaseDurationMs): string {
  if (ms === null) return INDEFINITELY;
...
export interface LeasePolicy {
  /** Finite default applied when a renew request omits a duration. */
  defaultMs: number;
  /** Hard maximum; `null` means indefinite leases may be requested. */
  maxMs: LeaseDurationMs;
}
 ...
 */
export function validateLeasePolicy(policy: LeasePolicy, latestStartMs: number): LeasePolicy {
  ...
  if (policy.maxMs !== null && policy.defaultMs > policy.maxMs) {
    throw new Error(
      `--lease ${formatLeaseDuration(policy.defaultMs)} exceeds --max-lease ${formatLeaseDuration(policy.maxMs)}`,
    );
  }
  if (leaseExpiryAt(latestStartMs, policy.defaultMs) === null) {
    throw new Error(
      `--lease ${formatLeaseDuration(policy.defaultMs)} cannot produce a valid expiry date from now`,
    );
  }
  if (policy.maxMs !== null && leaseExpiryAt(latestStartMs, policy.maxMs) === null) {
    throw new Error(
      `--max-lease ${formatLeaseDuration(policy.maxMs)} cannot produce a valid expiry date from now`,
    );
  }
  return policy;
}
