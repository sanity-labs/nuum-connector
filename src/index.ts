...
function startCommand(
  ...
  const finish = (frame: UplinkResultFrame): void => {
    ...
    if (child.exitCode !== null || child.signalCode !== null) clearKillTimer();
    entry.flow?.dispose();
    safeSend(ws, frame);
  };
...
async function handleRenew(
  ...
  spaceId: string,
  message: string,
  requestedLeaseDurationMs: unknown,
): Promise<void> {
  ...
  const requestedLabel =
    requestedLeaseDurationMs === undefined
      ? "default"
      : isValidLeaseDurationMs(requestedLeaseDurationMs)
        ? formatLeaseDuration(requestedLeaseDurationMs)
        : "invalid";
  console.log(
    `[renew:${commandId}] space=${spaceId} requested=${requestedLabel} reason=${JSON.stringify(message)}`,
  );
  try {
    const r = await authority.renew(spaceId, message, requestedLeaseDurationMs);
    if (r.ok) {
      console.log(
        `[renew:${commandId}] pending created, lease=${formatLeaseDuration(r.leaseDurationMs)}, notification sent`,
      );
      safeSend(ws, {
        type: "renew_result",
        commandId,
...
function handleAuth(
  ...
  if (r.ok) {
    ...
    console.log(
      `[auth:${commandId}] space=${spaceId} authorized lease=${formatLeaseDuration(r.leaseDurationMs)} expires=${until}`,
    );
    safeSend(ws, {
      type: "auth_result",
      commandId,
...
function main() {
  ...
  let leaseArg = "8h";
  let maxLeaseArg: string | undefined;
  ...
  const valueAfter = (i: number): string => {
    const next = args[i + 1];
  ...
  if (authMode !== undefined) {
    ...
    let defaultLeaseMs: number;
    let maxLeaseMs: LeaseDurationMs;
    try {
      ...
      defaultLeaseMs = parsedDefault as number;
      maxLeaseMs =
